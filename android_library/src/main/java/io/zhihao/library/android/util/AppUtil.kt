...
object AppUtil {
    fun isDebug(): Boolean {
        return try {
            ZLibrary.isDebug() || this.isAppDebug(ZLibrary.getAppContext().packageName)
        } catch (e: Exception) {
            e.printStackTrace()
            false
        }
    ...
    fun getString(stringRes: Int, formatArgs: Any? = null): String =
        ZLibrary.getAppContext().getString(
            stringRes,
            formatArgs
        )
    ...
    fun getAppVersionName(): String? =
        this.getAppVersionName(ZLibrary.getAppContext().packageName)
    ...
    fun installApp(file: File, authority: String): Boolean {
        return if (FileUtil.isFileExists(file)) {
            try {
                this.startActivity(
                    IntentUtil.getInstallAppIntent(file, authority, true) ?: return false
                )
                true
            } catch (e: Exception) {
                e.printStackTrace()
